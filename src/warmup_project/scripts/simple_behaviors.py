...
def full_scan_recieved(msg_avoid):
    """Process data from laser scanner, msg is of type sensor_msgs/LaserScan"""
    #print "front scan received"
    left_obstacles = {}
    right_obstacles = {}
    ...
    global avoid_angle
    for i in range(89):
        #print msg_avoid.ranges[i]
        if msg_avoid.ranges[i] > 0 and msg_avoid.ranges[i] < 3:
            left_obstacles[msg_avoid.ranges[i]] = i
        if msg_avoid.ranges[i+270] > 0 and msg_avoid.ranges[i+270] < 3:
            right_obstacles[msg_avoid.ranges[i+270]] = i+270
        if len(left_obstacles) > len(right_obstacles):
            avoid_force = min(left_obstacles.keys())
            avoid_angle = left_obstacles[avoid_force]
        elif len(left_obstacles) < len(right_obstacles):
            avoid_force = min(right_obstacles.keys())
            avoid_angle = right_obstacles[avoid_force]
        else:
            avoid_force = 1.0
            avoid_angle = 0
...
def obstacle_avoid():
    """The publisher for a Neato Controller"""
    pub_avoid = rospy.Publisher('cmd_vel', Twist, queue_size=10) #we're publishing to cmd_vel with a type Twist
    sub_avoid = rospy.Subscriber('scan', LaserScan, full_scan_recieved)
    ...
    while not rospy.is_shutdown():
        ...
        if avoid_force > 1.0:
            print 'nope'
            msg_avoid = Twist(Vector3(0.1,0.0,0.0), Vector3(0.0,0.0,0.0))
        else:
            print 'yikes'
            mag_turn = 0.005*avoid_angle
            if avoid_angle < 90:
                msg_avoid = Twist(Vector3(0.1,0.0,0.0), Vector3(0.0,0.0,-mag_turn))
            else:
                msg_avoid = Twist(Vector3(0.1,0.0,0.0), Vector3(0.0,0.0,mag_turn))

        #msg = Twist(Vector3(0.0, 0.0, 0.0), Vector3(0.0, 0.0, 2.0))
        #msg = Twist(angular=Vector3(z-2.0))
        pub_avoid.publish(msg_avoid)
        r.sleep()
...
if __name__ == '__main__':
    ...
    try:
        # while mean_distance < 0.98 or mean_distance > 1.02:
        #     wall_approach()
        # wall_follow()
        people_follow()
